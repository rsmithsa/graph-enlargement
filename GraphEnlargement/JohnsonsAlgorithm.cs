//-----------------------------------------------------------------------
// <copyright file="JohnsonsAlgorithm.cs" company="Richard Smith">
...
namespace GraphEnlargement
{
    ...
    public class JohnsonsAlgorithm<TVertex>
    {
        ...
        public IList<TVertex[]> Cycles
        {
            get
            {
                return this.cycles.Select(x => x.DropLast().Select(y => this.verticesMap[y]).ToArray()).ToList();
            }
        }
        ...
        private bool Circuit(int vertex, int s, BidirectionalGraph<TVertex, Edge<TVertex>> subGraph, Stack<int> stack)
        {
            ...
            foreach (var edge in subGraph.OutEdges(this.verticesMap[vertex]))
            {
                ...
                else
                {
                    ...
                    {
                        if (this.Circuit(target, s, subGraph, stack))
                        {
                            f = true;
                        }
                    }
