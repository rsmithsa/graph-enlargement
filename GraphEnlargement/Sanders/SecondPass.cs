...
namespace GraphEnlargement.Sanders
{
    ...
    public class SecondPass : IGraphEnlargementAlgorithm
    {
            ...
            where TVertex : class
        {
            ...
            HashSet<TVertex> cycles, noCycles;
            GetCycles(result, out cycles, out noCycles);
        ...
        private static void GetCycles<TVertex>(BidirectionalGraph<TVertex, Edge<TVertex>> inputGraph, out HashSet<TVertex> cycles, out HashSet<TVertex> noCycles)
            where TVertex : class
        {
            ...
            cycles = new HashSet<TVertex>(grouped.Where(x => x.Count() > 1).SelectMany(x => x));
            noCycles = new HashSet<TVertex>(grouped.Where(x => x.Count() == 1).SelectMany(x => x));
        }
    }
}
